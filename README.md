...
请假系统 Docker 服务器版。页面使用保存的原站外观，本地后端负责提交和记录持久化；原站记录快照与本地记录在“我的申请”中合并显示。

说明：原站当前缺失多个 Vue 动态分包，直接启用原运行时会显示“连接服务器超时”。此版本禁用了不可用的原运行时，改用本地静态交互脚本；登录页只需输入学号用于按学号隔离记录，密码和验证码只保留原外观，不做原站校验，也不会保存。
